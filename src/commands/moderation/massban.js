  ...
  async execute(interaction) {
    ...
    collector.on('collect', async (i) => {
      if (i.user.id !== interaction.user.id) {
        await i.reply({ content: 'Não podes interagir com este botão.', ephemeral: true });
      }
      ...
      if (i.customId === 'confirm_ban') {
        ...
        return i.update({ content: 'Ban cancelado.', components: [] });
      }
    });

    collector.on('end', (collected) => {
      if (collected.size === 0) {
        return interaction.editReply({ content: '**Ban timed out**. Nenhum membro foi banido.', components: [] });
      }
    });
  },
};
